...
`apps/c2s-public` Group Finder; shared `packages/*` including
...
only (no Vercel). Inventory lives in Studio at `/inventory`. Tract Tracker and
the standalone `apps/inventory` app have been **sunset** and removed.
