...
import { LoaderCircle, QrCode, Calendar, ArrowRight, RefreshCw, Utensils, PlusCircle, Clock, History, CheckCircle2 } from 'lucide-react';
import type { Booking, Room, AttendanceRecord, MealStub } from '@/lib/types';
...
export function ViewerDashboard() {
    ...
    const combinedQrUrl = combinedData ? `https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=${encodeURIComponent(combinedData)}` : '';
    ...
    const { data: attendanceHistory, isLoading: attendanceLoading } = useCollection<AttendanceRecord>(attendanceQuery);
    ...
    const { data: mealStubsHistory, isLoading: mealStubsLoading } = useCollection<MealStub>(mealStubsQuery);

    const upcomingBookings = bookings
        ?.map(b => ({ ...b, start: (b.start as any).toDate(), end: (b.end as any).toDate() }))
        .filter(b => b.status === 'Approved' && (isToday(b.start) || isAfter(b.start, new Date())))
        .slice(0, 3) || [];
    ...
    const isLoading = bookingsLoading || roomsLoading || attendanceLoading || mealStubsLoading;
            ...
            <TabsContent value="room">
                <Card className="w-full">
                    <CardHeader className="flex flex-row items-center justify-between space-y-0">
                        <div>
                            <CardTitle className="font-headline text-lg flex items-center gap-2"><Calendar className="h-4 w-4" /> Upcoming Bookings</CardTitle>
                            <CardDescription>Your next three approved reservations.</CardDescription>
                        </div>
                        ...
                        {isLoading ? (
                            ...
                            <div className="space-y-4">
                                {upcomingBookings.map(booking => (
                                    <div key={booking.id} className="flex items-center justify-between p-4 rounded-lg bg-secondary/30 border border-secondary">
                                        <div>
                                ...
                                <Button variant="link" asChild className="w-full">
                                    <Link href="/rooms">View Calendar <ArrowRight className="ml-2 h-4 w-4" /></Link>
                                </Button>
                            </div>
                        ) : (
                            <div className="flex flex-col items-center justify-center py-10 text-center">
                                <Calendar className="h-10 w-10 text-muted-foreground/30 mb-2" />
                                <p className="text-muted-foreground">You have no upcoming approved bookings.</p>
                                <Button variant="link" asChild className="mt-2">
                                    <Link href="/rooms">Open Calendar <ArrowRight className="ml-2 h-4 w-4" /></Link>
                                </Button>
                            </div>
                        )}
                    </CardContent>
