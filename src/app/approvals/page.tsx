...
const KanbanCard = ({ request, onUpdateStatus, canManage, onClick, requesterWorker }: { request: ApprovalRequest, onUpdateStatus: (request: ApprovalRequest, status: 'Approved' | 'Rejected') => void, canManage: boolean, onClick: (request: ApprovalRequest) => void, requesterWorker?: Worker | null }) => {
...
const KanbanColumn = ({ title, requests, onUpdateStatus, checkCanManage, onCardClick, icon, workers }: { title: string, requests: ApprovalRequest[], onUpdateStatus: (request: ApprovalRequest, status: 'Approved' | 'Rejected') => void, checkCanManage: (request: ApprovalRequest) => boolean, onCardClick: (request: ApprovalRequest) => void, icon?: React.ReactNode, workers?: Worker[] }) => {
    return (
...
export default function ApprovalsPage() {
    ...
    const handleUpdateRequestStatus = (request: ApprovalRequest, status: 'Approved' | 'Rejected') => {
        ...
        if (request.type === 'Ministry Change' && status === 'Approved') {
            if (request.status === 'Pending Outgoing Approval') {
                updateDocumentNonBlocking(doc(firestore, "approvals", request.id), {
                    status: 'Pending Incoming Approval',
                    outgoingApproved: true
                });
                return;
            }
            // If it's already Pending Incoming Approval, it proceeds to final Approved status below.
        }
