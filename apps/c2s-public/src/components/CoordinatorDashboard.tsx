...
const CoordDataContext = createContext<CoordinatorDashboardData | null>(null);

function useCoordData(): CoordinatorDashboardData {
    const data = useContext(CoordDataContext);
    if (!data) throw new Error('CoordDataContext is missing a provider');
    return data;
}
...
function CoordReportsTab({ clusterName, groups, reportsContent }: {
    ...
    const { mentors: COORD_MENTORS, potentialMentees: COORD_POTENTIAL_MENTEES } = useCoordData();
        ...
        <div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mt-6 mb-10">
                {[
                    ...
                    { value: groups.filter(g => g.status === 'Open').length,            label: 'Open Groups',      sub: 'Accepting now',     color: '#1971c2' },
                ].map(s => (
...
function CoordDashboardNotifications() {
    const { notifications: COORD_NOTIFICATIONS } = useCoordData();
...
export default function CoordinatorDashboard({
    ...
    const COORD_POTENTIAL_MENTEES: CoordPotentialMentee[] = data.potentialMentees;
    const COORD_MENTORS: CoordMentor[] = data.mentors;
    const COORD_GROUPS: CoordGroup[] = data.groups;
    const COORD_NOTIFICATIONS: DashboardNotification[] = data.notifications;
    const { user } = useAuth();
    ...
    return (
        <CoordDataContext.Provider value={data}>
