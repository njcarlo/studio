...
// Mentor dashboard data, loaded on the server and provided by the page.
const MentorDataContext = createContext<MentorDashboardData | null>(null);

function useMentorData(): MentorDashboardData {
    const data = useContext(MentorDataContext);
...
const ClusterReportContext = createContext<{
    mentors: Parameters<typeof chMentorReportData>[0];
    coordinators: Parameters<typeof chCoordReportData>[0];
    clusterGroups: Parameters<typeof chBarangayData>[0];
} | null>(null);
...
/* ── MenteesTab ──────────────────────────────────────────────────────────────── */
function MenteesTab({ activeMentees }: { activeMentees: Mentee[] }) {
    const { inactiveMentees: INACTIVE_MENTEES, groups } = useMentorData();
    const [viewing, setViewing] = useState<Mentee | null>(null);
    const [showDevotional, setShowDevotional] = useState<Mentee | null>(null);

    return (
        <>
            {viewing && <MenteeProfileModal mentee={viewing} onClose={() => setViewing(null)} />}
            {showDevotional && <DevotionalProgressModal mentee={showDevotional} groups={groups} onClose={() => setShowDevotional(null)} />}
...
function EndorsedTab() {
    const {
        endorsedGroups: ENDORSED_GROUPS,
        endorsedWorkers: ENDORSED_WORKERS,
        groups,
        activeMentees,
    } = useMentorData();
    const [viewingMembers, setViewingMembers] = useState<EndorsedGroup | null>(null);
    const [editingGroup, setEditingGroup] = useState<EndorsedGroup | null>(null);
    const [showCreate, setShowCreate] = useState(false);
    const [viewingWorker, setViewingWorker] = useState<EndorsedWorker | null>(null);
    const [endorseMentee, setEndorseMentee] = useState<EndorsedWorker | null>(null);

    return (
        <>
            {viewingMembers && (
                <EndorsedViewMembersModal
                    group={viewingMembers}
                    members={activeMentees.filter((m) => m.assignedGroup === viewingMembers.name)}
                    groups={groups}
            ...
            {viewingWorker  && <MenteeProfileModal mentee={viewingWorker} onClose={() => setViewingWorker(null)} />}
...
function MentorDashboard() {
    ...
    /**
     * Accepting a potential mentee advances the join request through the
     * approval engine; the server revalidates and the refreshed page shows
     * them under active mentees.
     */
    function handleMenteeAccepted(pm: PotentialMentee) {
        startTransition(async () => {
            await acceptMentee(pm.id);
            router.refresh();
        });
    }

    const allPotentialMentees = data.potentialMentees;
    const allActiveMentees = data.activeMentees;

    // Groups this mentor leads.
    const myGroups = user?.groupId
        ? data.groups.filter((g) => g.id === user.groupId)
        : data.groups;
    ...
    return (
        <div className="flex min-h-screen" style={{ background: '#EEF2F7' }}>
            ...
            {viewingGroup && (
                <ViewMembersModal
                    group={viewingGroup}
                    members={data.groupMembers[viewingGroup.name] ?? []}
            ...
            <aside className={`w-56 shrink-0 bg-[#f4f5f7] border-r border-gray-200 flex flex-col pt-6 pb-4 fixed top-16 bottom-0 left-0 z-40 transition-transform duration-200 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
                ...
                <nav className="px-3 flex flex-col gap-1">
                    ...
                    >
                        ...
                        Endorsed
                        <span className="ml-auto text-[11px] font-semibold w-5 h-5 flex items-center justify-center rounded-full" style={{ background: '#dde0f5', color: '#6366c1' }}>
                            {data.endorsedWorkers.length + data.endorsedGroups.reduce((sum, g) => sum + g.members, 0)}
                        </span>
                    </button>
                </nav>
            </aside>
...
function CHReportsCharts() {
    const clusterData = useContext(ClusterReportContext);
...
export default function DashboardView({ data }: { data: DashboardData }) {
    ...
    // The page redirects unauthenticated callers server-side; this is a guard
    // for the brief window after a client-side logout.
    if (!user) return null;
    ...
    if (data.role === 'ministry_head') {
        return (
            <>
                <DashNav onLogout={handleLogout} />
                <div className="pt-16">
                    <DepartmentHeadDashboard data={data} />
                </div>
            </>
        );
    }

    if (data.role === 'cluster_head') {
        return (
            <>
                <DashNav onLogout={handleLogout} />
                <div className="pt-16">
    ...
    return (
        <MentorDataContext.Provider value={data}>
            <DashNav onLogout={handleLogout} />
            <div className="pt-16">
                <MentorDashboard />
            </div>
