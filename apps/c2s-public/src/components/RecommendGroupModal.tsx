...
import { BARANGAYS, type C2SGroup } from '@/lib/data';
...
export default function RecommendGroupModal({ menteeName, groups, onClose, onConfirm }: Props) {
    ...
    // Open groups in the chosen barangay, falling back to every open group so
    // the step is never empty when a barangay has no group yet.
    const inBarangay = groups.filter((g) => g.status === 'Open' && g.barangay === barangay);
    const suggestedGroups = inBarangay.length > 0
        ? inBarangay
        : groups.filter((g) => g.status === 'Open');

    function handleSelectGroup(g: C2SGroup) {
        setSelectedGroup(g);
                            ...
                            <div className="flex flex-col gap-4">
                                ...
                                <div className="flex flex-col gap-3">
                                    {suggestedGroups.map((g) => (
                                        <div key={g.id} className="flex items-center justify-between border border-gray-200 rounded-xl px-5 py-4 bg-[#f8f9fc]">
                                            <div>
                                                <p className="font-semibold text-gray-900 text-sm">{g.name}</p>
                                                <p className="text-xs text-gray-400 mt-0.5">Mentor: {g.leader}</p>
