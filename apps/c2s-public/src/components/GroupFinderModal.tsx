...
import { AGE_GROUPS, MEETUP_DAYS, BARANGAYS, type C2SGroup } from '@/lib/data';
import Link from 'next/link';

// Load map only on client
const GroupFinderMap = dynamic(() => import('./GroupFinderMap'), { ssr: false });
    ...
    return (
        <div className="flex flex-col gap-1 min-w-0">
            ...
            <div className="relative">
                ...
                <div className="absolute right-2 top-1/2 -translate-y-1/2"><ChevronDown /></div>
            </div>
        </div>
    );
}

function GroupCard({ group, selected, onSelect }: { group: C2SGroup; selected: boolean; onSelect: () => void }) {
    return (
        <div
            onClick={onSelect}
            className={`bg-white rounded-2xl border-2 p-4 flex flex-col gap-2 cursor-pointer transition-all ${
                selected ? 'border-[#2dc7be] shadow-md' : 'border-gray-100 hover:border-gray-200 hover:shadow-sm'
            }`}
        >
            {/* Tags + status */}
...
export default function GroupFinderModal({ groups, onClose }: Props) {
    ...
    const filtered = useMemo(() => {
        return groups.filter((g) => {
            ...
            const matchBarangay = barangay === 'All' || g.barangay === barangay;
            return matchSearch && matchAge && matchDay && matchBarangay;
        });
    }, [groups, search, ageGroup, meetupDay, barangay]);
    ...
    return (
        ...
        >
            <div className="bg-[#f8f9fc] rounded-2xl shadow-2xl w-full max-w-6xl max-h-[92vh] flex flex-col overflow-hidden">
                ...
                <div className="bg-white px-6 pt-6 pb-4 border-b border-gray-100 shrink-0">
                    ...
                    {/* c2s logo + "Group Finder" title */}
                    <div className="flex items-center gap-3 mb-1">
                        <img src="/c2s.png" alt="C2S" style={{ width: 48, height: 48, objectFit: 'contain' }} />
                        <h1 className="text-2xl font-black text-gray-900">Group Finder</h1>
                    </div>
                    <p className="text-sm text-gray-400">
                        Discover a Connect2Souls group near you in Dasmariñas City — built for real friendships,
                        growth in faith, and a place to belong.
                    </p>
                </div>
                    ...
                    <div className="w-full lg:w-[55%] flex flex-col overflow-hidden border-r border-gray-100">
                        ...
                        <div className="bg-white px-4 py-4 border-b border-gray-100 shrink-0">
                            ...
                            <div className="grid grid-cols-3 gap-2">
                                ...
                                <Select label="Barangay/Subdivision" value={barangay}  onChange={setBarangay}
                ...
                <button
                    onClick={onClose}
                    className="absolute top-4 right-4 w-8 h-8 flex items-center justify-center rounded-full bg-gray-100 hover:bg-gray-200 text-gray-500 text-lg leading-none transition-colors"
                    aria-label="Close"
                >
                    ×
                </button>
