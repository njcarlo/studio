...
import type {
    C2SCoordinator, ClusterMentor, ClusterPotentialMentee, Mentee,
...
const ClusterDataContext = createContext<ClusterHeadDashboardData | null>(null);

function useClusterData(): ClusterHeadDashboardData {
    const data = useContext(ClusterDataContext);
    if (!data) throw new Error('ClusterDataContext is missing a provider');
    return data;
}
...
export const chCoordReportData = (coordinators: C2SCoordinator[]) =>
    coordinators.map(c => ({ name: c.name.split(' ')[0], assigned: c.assignedPotential, pending: c.pendingAssignments }));
export const chBarangayData = (groups: ClusterGroupPin[]) =>
    groups.map(g => ({ name: g.barangay, members: g.members }));
...
function chGroupsToMapGroups(groups: ClusterGroupPin[]): ClusterMapGroup[] {
    return groups.map(g => ({
...
function MapTab() {
    const { clusterGroups: CH_CLUSTER_GROUPS } = useClusterData();
...
function CHReportsTab({ clusterName, reportsContent }: { clusterName: string; reportsContent?: ReactNode }) {
    const { activeMentees: CH_ACTIVE_MENTEES, clusterGroups: CH_CLUSTER_GROUPS, coordinators: CH_COORDINATORS, mentors: CH_MENTORS, potentialMentees: CH_POTENTIAL_MENTEES } = useClusterData();
...
function DashboardNotifications() {
    const { notifications: CH_NOTIFICATIONS } = useClusterData();
...
export default function ClusterHeadDashboard({
    ...
    data: ClusterHeadDashboardData;
    onLogout: () => void;
    reportsContent?: ReactNode;
}) {
    const CH_COORDINATORS: C2SCoordinator[] = data.coordinators;
    const CH_MENTORS: ClusterMentor[] = data.mentors;
    const CH_POTENTIAL_MENTEES: ClusterPotentialMentee[] = data.potentialMentees;
    const CH_ACTIVE_MENTEES: Mentee[] = data.activeMentees;
    const INACTIVE_MENTEES: InactiveMentee[] = data.inactiveMentees;
    const CH_CLUSTER_GROUPS: ClusterGroupPin[] = data.clusterGroups;
    const CH_NOTIFICATIONS: DashboardNotification[] = data.notifications;
    const { user } = useAuth();
    ...
    return (
        <ClusterDataContext.Provider value={data}>
