...
import type {
    OutreachCluster, MHCoordinator, MHMentor, MHPotentialMentee,
...
// Dashboard data arrives from the server through the page; a context keeps the
// module-scope tab components below reading it without prop-drilling.
const MHDataContext = createContext<MinistryHeadDashboardData | null>(null);

function useMHData(): MinistryHeadDashboardData {
    const data = useContext(MHDataContext);
    if (!data) throw new Error('MHDataContext is missing a provider');
    return data;
}
...
function MHDashboardNotifications() {
    const { notifications: MH_NOTIFICATIONS } = useMHData();
...
// ─── Clusters Tab ─────────────────────────────────────────────────────────────
function MentorProfileView({ mentor, cluster, onClose }: { mentor: MHMentor; cluster: OutreachCluster; onClose: () => void }) {
    const { activeMentees: MH_ACTIVE_MENTEES_LIST, mentors: MH_ALL_MENTORS } = useMHData();
...
function ClusterDetailPanel({ cluster, onClose }: { cluster: OutreachCluster; onClose: () => void }) {
    const { mentors: MH_ALL_MENTORS } = useMHData();
    const [viewingMentor, setViewingMentor] = useState<MHMentor | null>(null);
...
function ClustersTab() {
    const { clusters: MH_CLUSTERS } = useMHData();
...
function CoordinatorsTab() {
    const { coordinators: MH_COORDINATORS } = useMHData();
...
function PotentialMenteesTab() {
    const { potentialMentees: MH_POTENTIAL_MENTEES } = useMHData();
...
function ActiveMenteesTab() {
    const { activeMentees: MH_ACTIVE_MENTEES_LIST, clusters: MH_CLUSTERS } = useMHData();
    const [clusterFilter, setClusterFilter] = useState('All');
...
const clusterPerf = (clusters: OutreachCluster[]) =>
        ...
        <div>
            ...
            {(reportType === 'cluster' || reportType === 'coordinator' || reportType === 'mentor') && (
                <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6" style={{ boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                        <BarChart data={clusterPerf(MH_CLUSTERS)} margin={{ top: 10, right: 20, left: -16, bottom: 0 }}>
                            <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" vertical={false}/>
...
export default function DepartmentHeadDashboard({ data }: { data: MinistryHeadDashboardData }) {
    const MH_CLUSTERS: OutreachCluster[] = data.clusters;
    const MH_COORDINATORS: MHCoordinator[] = data.coordinators;
    const MH_ALL_MENTORS: MHMentor[] = data.mentors;
    const MH_POTENTIAL_MENTEES: MHPotentialMentee[] = data.potentialMentees;
    const MH_ACTIVE_MENTEES_LIST: MHActiveMentee[] = data.activeMentees;
    const MH_NOTIFICATIONS: DashboardNotification[] = data.notifications;
    ...
    return (
        <MHDataContext.Provider value={data}>
