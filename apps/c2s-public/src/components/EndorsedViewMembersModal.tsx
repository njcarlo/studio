...
import type { C2SGroup, EndorsedGroup, Mentee } from '@/lib/data';
...
interface Props {
    group: EndorsedGroup;
    members: Mentee[];
    /** Groups a member can be transferred into. */
    groups: C2SGroup[];
    onClose: () => void;
}
...
export default function EndorsedViewMembersModal({ group, members: allMembers, groups, onClose }: Props) {
    const [search, setSearch]       = useState('');
    const [viewProfile, setViewProfile] = useState<Mentee | null>(null);
    const [updating, setUpdating]   = useState<Mentee | null>(null);

    const members = allMembers.filter(m => m.name.toLowerCase().includes(search.toLowerCase()));
    const details = groups.find(g => g.id === group.id);

    if (viewProfile) {
        return <MenteeProfileModal mentee={viewProfile} onClose={() => setViewProfile(null)} />;
    }
    if (updating) {
        return <DevotionalProgressModal mentee={updating} groups={groups} onClose={() => setUpdating(null)} />;
    }
            ...
            <div className="fixed top-0 right-0 bottom-0 z-[101] w-[480px] max-w-full bg-white shadow-2xl flex flex-col overflow-hidden">
                ...
                <div className="mx-6 mt-5 rounded-xl border border-gray-100 bg-[#f8f9fc] grid grid-cols-2 divide-x divide-y divide-gray-100">
                    {[
                        ...
                        { label: 'GROUP TYPE',       value: details?.tags.map(t => t.label).join(' · ') || 'Mentor' },
                        { label: 'MEETING SCHEDULE', value: details?.schedule || '—' },
                        { label: 'FREQUENCY',        value: details?.meetupDay ? `Every ${details.meetupDay}` : 'Weekly' },
                        { label: 'CHURCH SATELLITE', value: details?.location || '—' },
