...
import {
  ...
  User, Calendar, ArrowUpCircle, ArrowDownCircle, QrCode, Upload, ShieldAlert
} from 'lucide-react';
...
function borrowerDisplayName(b: Borrowing) {
  if (b.borrowerName) return b.borrowerName;
  if (b.borrower) return `${b.borrower.firstName} ${b.borrower.lastName}`;
  return 'Unknown';
}

function borrowerEmail(b: Borrowing) {
  return b.borrower?.email ?? '';
}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.75rem' }}>
        ...
        <div style={{ display: 'flex', gap: '0.5rem' }}>
          <button className="btn btn-outline" style={{ height: '36px', fontSize: '0.8125rem', gap: '0.35rem' }} onClick={() => setScanModalOpen(true)}>
            <QrCode size={14} /> Scan QR
          </button>
          <button className="btn btn-primary" style={{ height: '36px', fontSize: '0.8125rem', gap: '0.35rem' }} onClick={() => setCheckoutModalOpen(true)}>
      ...
      <div className="stats-grid-3">
        {[
          { label: 'Total Borrowings', value: borrowings.length, icon: ClipboardList, color: '#3b5bdb', bg: '#eef2ff' },
      ...
      <div className="card" style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        <div className="action-bar">
          <div className="search-container">
            ...
            <input type="text" placeholder="Search item or borrower…" className="search-input" style={{ fontSize: '0.8125rem' }} value={search} onChange={e => setSearch(e.target.value)} />
          ...
          <div style={{ display: 'flex', gap: '0.35rem' }}>
            {statusOptions.map(s => (
              <button key={s.value} onClick={() => { setStatusFilter(s.value); setPage(1); }} style={{ padding: '0.3rem 0.75rem', fontSize: '0.78rem', fontWeight: 500, border: '1px solid', borderRadius: '8px', cursor: 'pointer', borderColor: statusFilter === s.value ? '#3b5bdb' : '#e5e7eb', backgroundColor: statusFilter === s.value ? '#eef2ff' : '#f9fafb', color: statusFilter === s.value ? '#3b5bdb' : '#6b7280', fontFamily: 'inherit' }}>
                {s.label}
              </button>
            ))}
          </div>
          <button onClick={fetchBorrowings} className="btn btn-outline" style={{ height: '34px', padding: '0 0.75rem', gap: '0.35rem', fontSize: '0.8125rem' }}>
            <RefreshCw size={14} style={loading ? { animation: 'inv-spin 1s linear infinite' } : {}} />
          </button>
        </div>
              ...
              {loading ? (
                ...
                return (
                  <tr key={b.id} style={{ borderBottom: '1px solid #f3f4f6', backgroundColor: isOverdue ? '#fff9f9' : 'transparent' }}>
                    <td style={{ padding: '0.7rem 0.875rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                        ...
                        <div>
                          ...
                          <div style={{ fontSize: '0.68rem', color: '#9ca3af', fontFamily: 'monospace' }}>{b.item.inventoryCode || b.itemId.slice(0, 8).toUpperCase()}</div>
                        </div>
                      </div>
                    </td>
                    <td style={{ padding: '0.7rem 0.875rem', fontSize: '0.8125rem', fontWeight: 600, color: '#1a1a2e' }}>{b.quantity || 1}</td>
                    <td style={{ padding: '0.7rem 0.875rem' }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                        ...
                        <div>
                          <div style={{ fontSize: '0.8125rem', fontWeight: 500, color: '#1a1a2e' }}>{borrowerDisplayName(b)}</div>
                          {borrowerEmail(b) && <div style={{ fontSize: '0.68rem', color: '#9ca3af' }}>{borrowerEmail(b)}</div>}
                        </div>
                      </div>
                    </td>
                    ...
                    <td style={{ padding: '0.7rem 0.875rem' }}>
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.2rem 0.6rem', borderRadius: '9999px', fontSize: '0.75rem', fontWeight: 600, backgroundColor: stConfig.bg, color: stConfig.color }}>
                    ...
                    <td style={{ padding: '0.7rem 0.5rem' }}>
                      <div style={{ display: 'flex', gap: '0.35rem' }}>
                        {b.status === 'BORROWED' && (
                          <button onClick={() => setReturnModalId(b.id)} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.3rem', padding: '0.25rem 0.6rem', fontSize: '0.75rem', fontWeight: 600, border: '1px solid #10b981', borderRadius: '6px', backgroundColor: '#f0fdf4', color: '#16a34a', cursor: 'pointer', fontFamily: 'inherit' }}>
                            <ArrowUpCircle size={12} /> Return
                          </button>
                        )}
                        <button onClick={() => setQrItem({ id: b.item.id, name: b.item.name })} style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', width: '28px', height: '28px', border: '1px solid #e5e7eb', borderRadius: '6px', backgroundColor: '#fff', color: '#6b7280', cursor: 'pointer' }} title="View QR">
                          <QrCode size={13} />
                        </button>
                      </div>
                    </td>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontSize: '0.8rem', color: '#9ca3af' }}>
            Showing <strong style={{ color: '#374151' }}>{total === 0 ? 0 : (page - 1) * take + 1}</strong> – <strong style={{ color: '#374151' }}>{Math.min(page * take, total)}</strong> of <strong style={{ color: '#374151' }}>{total}</strong>
          </div>
          <div style={{ display: 'flex', gap: '0.35rem' }}>
            <button className="btn btn-outline" style={{ padding: '0.25rem 0.5rem', height: '30px', borderRadius: '6px' }} disabled={page === 1} onClick={() => setPage(p => p - 1)}>
              <ChevronLeft size={14} />
            </button>
            <span style={{ fontSize: '0.78rem', minWidth: '60px', textAlign: 'center', color: '#6b7280' }}>Page {page} of {totalPages}</span>
            <button className="btn btn-outline" style={{ padding: '0.25rem 0.5rem', height: '30px', borderRadius: '6px' }} disabled={page >= totalPages} onClick={() => setPage(p => p + 1)}>
      ...
      {qrItem && <QRModal isOpen={!!qrItem} onClose={() => setQrItem(null)} items={[qrItem]} />}
    </div>
  );
}

function CheckoutModal({ ministryId, preselectedItem, onClose, onSuccess }: {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        const { items: results } = await getItems(ministryId, { search: itemSearch, limit: 8 });
        setItems(results);
      } catch { }
    }, 300);
    return () => clearTimeout(timer);
  }, [itemSearch, ministryId]);

  const handleSubmit = async () => {
    if (!selectedItem || !borrowerName.trim()) { setError('Please select an item and enter borrower name.'); return; }
    if (selectedItem?.isApprovalRequired && !supervisorName.trim()) { setError('Ministry Head approval is required for this high-value item.'); return; }
    setLoading(true);
    setError('');
    try {
      await createBorrowing({
        ...
        checkoutNotes: selectedItem?.isApprovalRequired
          ? `Approved by: ${supervisorName.trim()}${notes ? ' - ' + notes : ''}`
          : (notes || undefined),
      });
      onSuccess();
    } catch (e: any) {
    ...
    <div className="modal-overlay" style={{ zIndex: 400 }}>
      <div className="modal-content" style={{ maxWidth: '520px' }}>
        ...
        <div className="modal-body" style={{ gap: '1rem' }}>
          ...
          <div>
            ...
            {selectedItem ? (
              ...
              <div style={{ position: 'relative' }}>
                <input type="text" className="form-control" placeholder="Search equipment by name…" value={itemSearch} onChange={e => setItemSearch(e.target.value)} />
                {items.length > 0 && (
                  <div style={{ position: 'absolute', top: '100%', left: 0, right: 0, zIndex: 50, backgroundColor: '#fff', border: '1px solid #e5e7eb', borderRadius: '8px', boxShadow: '0 4px 12px rgba(0,0,0,0.1)', marginTop: '2px', overflow: 'hidden' }}>
                    {items.map((item: any) => (
                      <button key={item.id} onClick={() => { setSelectedItem(item); setItemSearch(''); setItems([]); }} style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', width: '100%', padding: '0.5rem 0.75rem', border: 'none', background: 'none', cursor: 'pointer', textAlign: 'left', fontFamily: 'inherit' }} onMouseEnter={e => (e.currentTarget.style.backgroundColor = '#f9fafb')} onMouseLeave={e => (e.currentTarget.style.backgroundColor = 'transparent')}>
          ...
          <div className="form-row-2col">
            <div>
              ...
              <input type="number" className="form-control" min={1} disabled={!selectedItem} value={quantity} onChange={e => setQuantity(Math.max(1, Number(e.target.value)))} />
              {selectedItem && <div style={{ fontSize: '0.7rem', color: '#6b7280', marginTop: '0.2rem' }}>Available: {selectedItem.quantity}</div>}
            </div>
            <div>
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn-primary" onClick={handleSubmit} disabled={loading || !selectedItem || !borrowerName.trim()}>
            {loading ? <><RefreshCw size={14} style={{ animation: 'inv-spin 1s linear infinite' }} /> Processing…</> : <><ArrowDownCircle size={14} /> Confirm Checkout</>}
          </button>
        </div>
      </div>
    </div>
...
function ReturnModal({ borrowingId, onClose, onSuccess }: {
  borrowingId: string;
  onClose: () => void;
  onSuccess: () => void;
}) {
  ...
  const handleReturn = async () => {
    ...
    try {
      await returnBorrowing(borrowingId, {
        ...
        returnNotes: damaged ? damageNotes : notes || undefined,
      });
      onSuccess();
    } catch (e: any) {
  ...
  return (
    <div className="modal-overlay" style={{ zIndex: 400 }}>
      <div className="modal-content" style={{ maxWidth: '480px' }}>
        ...
        <div className="modal-body" style={{ gap: '1rem' }}>
          ...
          {borrowing && (
            <div style={{ padding: '0.75rem', borderRadius: '8px', backgroundColor: '#f9fafb', border: '1px solid #e5e7eb', display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              <div style={{ fontSize: '0.8125rem', color: '#374151' }}><strong>Borrower:</strong> {borrowerDisplayName(borrowing)}</div>
              <div style={{ fontSize: '0.8125rem', color: '#374151' }}><strong>Quantity:</strong> {borrowing.quantity || 1}</div>
          ...
          <div>
            <label className="form-label" style={{ marginBottom: '0.5rem' }}>Return Photos (optional)</label>
            <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
              ...
              <label className="btn btn-outline" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', width: '60px', height: '60px', padding: 0, borderRadius: '8px', cursor: 'pointer', margin: 0, backgroundColor: '#f9fafb' }}>
                {uploadingImage ? <RefreshCw size={18} style={{ animation: 'inv-spin 1s linear infinite' }} /> : <Upload size={18} color="#9ca3af" />}
                <input type="file" accept="image/*" style={{ display: 'none' }} disabled={uploadingImage} onChange={async e => {
                  if (!e.target.files?.length) return;
                  setUploadingImage(true);
                  try { const url = await uploadItemPhoto(e.target.files[0]); setPhotos(p => [...p, url]); }
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', cursor: 'pointer', padding: '0.75rem', borderRadius: '8px', border: `1px solid ${damaged ? '#fecaca' : '#e5e7eb'}`, backgroundColor: damaged ? '#fef2f2' : '#f9fafb' }}>
            <input type="checkbox" checked={damaged} onChange={e => setDamaged(e.target.checked)} style={{ width: '16px', height: '16px', accentColor: '#dc2626', cursor: 'pointer' }} />
            <div>
        ...
        <div className="modal-footer">
          <button className="btn btn-outline" onClick={onClose}>Cancel</button>
          <button className="btn btn-primary" style={{ backgroundColor: damaged ? '#dc2626' : '#10b981', borderColor: damaged ? '#dc2626' : '#10b981' }} onClick={handleReturn} disabled={loading || (damaged && !damageNotes.trim())}>
            {loading ? <><RefreshCw size={14} style={{ animation: 'inv-spin 1s linear infinite' }} /> Processing…</> : <><ArrowUpCircle size={14} /> {damaged ? 'Return (Damaged)' : 'Confirm Return'}</>}
          </button>
        </div>
      </div>
    </div>
...
function BorrowScanModal({ onClose, onCheckout, onReturn }: {
  onClose: () => void;
  ...
  return (
    <div className="modal-overlay" style={{ zIndex: 400 }}>
      {isCameraOpen && <ScannerModal onClose={() => setIsCameraOpen(false)} onScan={(data) => { setQrInput(data); handleScanAPI(data); }} />}
        ...
        <div className="modal-body" style={{ gap: '1rem' }}>
          <div style={{ textAlign: 'center', padding: '1.5rem 1rem', backgroundColor: '#f9fafb', borderRadius: '10px', border: '2px dashed #e5e7eb' }}>
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', marginBottom: '1rem' }}>
              ...
              <button className="btn btn-primary" style={{ fontSize: '0.78rem' }} onClick={() => setIsCameraOpen(true)}>Start Camera Scanner</button>
            </div>
            <p style={{ fontSize: '0.8125rem', color: '#9ca3af', marginBottom: '1rem' }}>Or type the QR payload manually below:</p>
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <input type="text" className="form-control" value={qrInput} onChange={e => setQrInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleScanAPI(qrInput)} autoFocus />
              <button className="btn btn-primary" style={{ flexShrink: 0 }} onClick={() => handleScanAPI(qrInput)} disabled={scanning || !qrInput.trim()}>
                {scanning ? <RefreshCw size={14} style={{ animation: 'inv-spin 1s linear infinite' }} /> : 'Look Up'}
              </button>
            </div>
          </div>
            ...
            <div style={{ border: '1px solid #e5e7eb', borderRadius: '10px', overflow: 'hidden' }}>
              ...
              <div style={{ padding: '0.75rem 1rem', display: 'flex', gap: '0.5rem' }}>
                {scanResult.activeBorrowing ? (
                  <button className="btn btn-primary" style={{ flex: 1, backgroundColor: '#10b981' }} onClick={() => onReturn(scanResult.activeBorrowing.id)}>
                    <ArrowUpCircle size={14} /> Check IN (Return)
                  </button>
                ) : (
                  <button className="btn btn-primary" style={{ flex: 1 }} onClick={() => onCheckout(scanResult.item)}>
