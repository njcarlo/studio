...
import { lookupItemByQR, adjustStock } from '@/services/inventory-api';
...
export function StockScanModal({ onClose }: { onClose: () => void }) {
  ...
  const handleStockUpdate = async (action: 'Stock In' | 'Stock Out') => {
    ...
    setError('');
    try {
      const newQty = await adjustStock(scanResult.item.id, action, 1, 'Scanned via Mobile Action');
      setScanResult((prev: any) => ({
        ...prev,
        item: { ...prev.item, stock: newQty, quantity: newQty }
      }));
    } catch (e: any) {
    ...
    <div className="modal-overlay" style={{ zIndex: 400 }}>
      {isCameraOpen && (
        <ScannerModal
          ...
          onScan={(data) => { setQrInput(data); handleScanAPI(data); }}
        />
      )}
        ...
        <div className="modal-body" style={{ gap: '1rem' }}>
          <div style={{ textAlign: 'center', padding: '1.5rem 1rem', backgroundColor: '#f9fafb', borderRadius: '10px', border: '2px dashed #e5e7eb' }}>
            ...
            <div style={{ display: 'flex', gap: '0.5rem' }}>
              <input type="text" className="form-control" value={qrInput} onChange={e => setQrInput(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleScanAPI(qrInput)} placeholder="Item ID or Code" />
              ...
              <div style={{ padding: '1rem', backgroundColor: '#f9fafb', borderBottom: '1px solid #e5e7eb', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '1rem' }}>
                <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: '0.78rem', color: '#9ca3af', marginTop: '0.2rem' }}>{scanResult.item.inventoryCode || scanResult.item.id?.slice(0, 8)} · {scanResult.item.category?.name}</div>
                  </div>
                </div>
                <div style={{ textAlign: 'right' }}>
                  <div style={{ fontSize: '0.7rem', color: '#9ca3af', textTransform: 'uppercase', fontWeight: 700 }}>Current Stock</div>
                  <div style={{ fontSize: '1.25rem', fontWeight: 800, color: '#1a1a2e' }}>{scanResult.item.stock ?? scanResult.item.quantity}</div>
                </div>
              </div>
              <div style={{ padding: '0.75rem 1rem', borderTop: '1px solid #e5e7eb', display: 'flex', gap: '0.5rem' }}>
                <button className="btn btn-outline" style={{ flex: 1, borderColor: '#10b981', color: '#10b981' }} onClick={() => handleStockUpdate('Stock In')} disabled={scanning}>
                  <ArrowUpCircle size={14} /> Stock IN (+1)
                </button>
                <button className="btn btn-outline" style={{ flex: 1, borderColor: '#d97706', color: '#d97706' }} onClick={() => handleStockUpdate('Stock Out')} disabled={scanning || (scanResult.item.stock ?? scanResult.item.quantity) <= 0}>
