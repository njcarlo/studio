...
function parseRows(sheet: XLSX.WorkSheet): ParsedRow[] {
  ...
  for (let i = 0; i < Math.min(raw.length, 10); i++) {
    ...
    if (row.some((c: string) => c.includes('inventory code') || c.includes('items'))) {
      headerIdx = i;
      break;
    }
  }
  ...
  const colStatusCode = col('status code') !== -1 ? col('status code') : 9;
  ...
  for (let i = headerIdx + 1; i < raw.length; i++) {
    ...
    if (pdRaw) {
      if (typeof pdRaw === 'number') {
        const d = (XLSX as any).SSF?.parse_date_code?.(pdRaw);
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', maxWidth: '900px' }}>
      ...
      {step === 'upload' && (
        <div
          ...
          style={{ border: `2px dashed ${dragging ? 'var(--primary)' : 'var(--border)'}`, borderRadius: '12px', padding: '3rem', textAlign: 'center', cursor: 'pointer', backgroundColor: dragging ? 'var(--primary-light)' : 'var(--surface)', transition: 'all 0.15s' }}
        >
          <FileSpreadsheet size={48} style={{ margin: '0 auto 1rem', color: dragging ? 'var(--primary)' : 'var(--text-muted)', opacity: 0.6 }} />
          <p style={{ fontWeight: 600, color: 'var(--text-main)', marginBottom: '0.25rem' }}>Drop your Excel or CSV file here</p>
          <p style={{ fontSize: '0.8125rem', color: 'var(--text-muted)' }}>Supports .xlsx, .xls, .csv · Columns: Weight, Group, Category, Inventory Code, Items, Role, Location, Assigned to, Purchase Date, Status Code, Status, Status Details, Recommendation</p>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                  ...
                  {preview.slice(0, 100).map((row, i) => (
                    <tr key={i} style={{ borderBottom: '1px solid var(--border)' }}>
                      ...
                      <td style={{ padding: '0.5rem 0.875rem' }}>
                        <span style={{ display: 'inline-block', padding: '0.15rem 0.5rem', borderRadius: '999px', fontSize: '0.7rem', fontWeight: 600, backgroundColor: row.statusCode === 1 ? 'var(--success-bg)' : row.statusCode && row.statusCode > 1 ? 'var(--warning-bg)' : 'var(--border)', color: row.statusCode === 1 ? 'var(--success)' : row.statusCode && row.statusCode > 1 ? 'var(--warning)' : 'var(--text-muted)' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
            ...
            <button className="btn btn-primary" onClick={handleImport} disabled={importing} style={{ minWidth: '140px' }}>
              {importing
                ? <><Loader size={14} style={{ marginRight: '0.4rem', animation: 'inv-spin 1s linear infinite' }} /> Importing…</>
      ...
      {step === 'done' && result && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <div style={{ display: 'flex', gap: '0.75rem', justifyContent: 'flex-end' }}>
            ...
            <a href="/inventory/dashboard" className="btn btn-primary">View Inventory</a>
          </div>
        </div>
      )}
