...
import type { Booking, Room, Area, Worker, VenueElement } from "@studio/types";
...
export default function MasterviewPage() {
  ...
  const filteredBookings = useMemo(() => {
    ...
    if (viewMode === "upcoming") {
      ...
      result = result.filter((b) => isAfter(toJsDate(b.start), today));
      result.sort(
          ...
          <div className="flex items-center gap-3">
            <div className="bg-muted p-1 rounded-lg flex items-center border">
              <Button
                ...
                size="sm"
                className={cn(
                  "text-xs font-medium px-4 h-8 rounded-md transition-all",
                  viewMode === "all"
        ...
        {isLoading ? (
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-sm text-muted-foreground max-w-xs mt-2">
              No approved reservations match your current search or filter
              criteria.
            </p>
          </div>
        ) : (
              ...
              <Table>
                <TableHeader>
                  <TableRow className="hover:bg-transparent">
                    <TableHead className="w-[180px] font-black text-slate-500 text-[10px] uppercase tracking-[0.2em] h-14 px-6">
                      Venue
                    </TableHead>
                    <TableHead className="w-[200px] font-black text-slate-500 text-[10px] uppercase tracking-[0.2em] h-14 px-6">
                      Date & Time
                    </TableHead>
                    <TableHead className="font-black text-slate-500 text-[10px] uppercase tracking-[0.2em] h-14 px-6">
                      Event Details
                    </TableHead>
                    <TableHead className="w-[220px] font-black text-slate-500 text-[10px] uppercase tracking-[0.2em] text-center h-14 px-6">
                      Requirements
                    </TableHead>
                    ...
                    return (
                      <TableRow key={booking.id} className="group">
                        ...
                        <TableCell className="py-2 px-3 align-middle">
                          <div>
                            <p className="text-sm">
                              {format(toJsDate(booking.start), "MMM d, yyyy")}
                            </p>
                            <p className="text-xs text-muted-foreground">
                          ...
                          booking.requestedElements.length > 0 ? (
                            ...
                            <div className="flex flex-wrap gap-1 justify-center max-w-[150px]">
                              {booking.equipment_TV && (
                                <Badge
                                  variant="outline"
                                  className="text-[9px] h-4 text-blue-600"
                                >
                                  TV
                                </Badge>
                              )}
                              {booking.equipment_Mic && (
                                <Badge
                                  variant="outline"
                                  className="text-[9px] h-4 text-emerald-600"
                                >
                                  Mic
                                </Badge>
                              )}
              ...
              booking.requestedElements.length > 0 ? (
                booking.requestedElements.map((elId: string) => {
                  ...
                  return (
                    <div
                      key={elId}
                      className="flex items-center justify-between p-3 rounded-xl border bg-emerald-50/50 border-emerald-100 text-emerald-900"
                    >
                      <div className="flex items-center gap-3">
                ...
                <>
                  {booking.equipment_TV && (
                    <LegacyRow label="Television / Multimedia" />
                  )}
                  {booking.equipment_Mic && (
                    <LegacyRow label="Microphone / Audio" />
                  )}
                  {booking.equipment_Speakers && (
                    <LegacyRow label="Sound System / Speakers" />
                  )}
                </>
              ) : (
