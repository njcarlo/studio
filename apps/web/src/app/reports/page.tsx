      ...
      <div className="flex items-center gap-2 w-full lg:w-auto overflow-x-auto pb-2 lg:pb-0 scrollbar-hide">
        ...
        <Popover>
          <PopoverTrigger asChild>
            <Button
              variant="outline"
              className="flex-grow lg:flex-none lg:min-w-[200px] justify-start text-left font-normal text-sm h-9"
            >
...
function SummaryDashboard() {
  ...
  const { data: attendanceData, isLoading: aLoading } = useQuery({
    queryKey: ["attendance", { dateFrom: last30Start, dateTo: last30End }],
    queryFn: () =>
      getAttendanceRecords({ dateFrom: last30Start, dateTo: last30End }),
  });
  const { data: mealstubData, isLoading: mLoading } = useQuery({
    queryKey: ["meal-stubs", { dateFrom: last30Start, dateTo: last30End }],
    queryFn: () => getMealStubs({ dateFrom: last30Start, dateTo: last30End }),
  });
  ...
  const totalClockIns = useMemo(
    () => attendanceData?.filter((a) => a.type === "Clock In").length ?? 0,
    [attendanceData],
  );
  const claimedMeals = useMemo(
    () => mealstubData?.filter((m) => m.status === "Claimed").length ?? 0,
    [mealstubData],
  );
      ...
      <div>
        ...
        <div className="grid gap-4 grid-cols-1 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <KpiCard
            ...
            sub={`of ${mealstubData?.length ?? 0} issued`}
          />
          <KpiCard
...
function AttendanceReport({ dateRange }: { dateRange: DateRange }) {
  ...
  const handleExport = () => {
    ...
    exportCsv(
      ...
      ["Worker", "Type", "Time"],
      attendance.map((a) => [
        getWorkerName(a.workerProfileId),
        a.type,
        format(toJsDate(a.time), "Pp"),
      ]),
    );
  };
                  ...
                  attendance?.map((log) => (
                    <TableRow key={log.id}>
                      <TableCell className="font-medium">
                        {getWorkerName(log.workerProfileId)}
                      </TableCell>
                      <TableCell>
                        <Badge
                          variant="outline"
...
function MealStubsReport({ dateRange }: { dateRange: DateRange }) {
  const { data: mealstubs, isLoading } = useQuery({
    ...
    queryFn: () =>
      getMealStubs({ dateFrom: dateRange.start, dateTo: dateRange.end }),
  });
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      claimRate:
        mealstubs.length > 0
          ? Math.round((claimed / mealstubs.length) * 100)
          : 0,
    };
  }, [mealstubs]);
      ...
      <Card className="border-none sm:border shadow-none sm:shadow-sm">
        <CardHeader className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            ...
            <CardDescription className="text-xs">
              History of issued and claimed stubs
            </CardDescription>
          </div>
          <Button
            size="sm"
...
function MealAllocationsReport({ dateRange }: { dateRange: DateRange }) {
  ...
  const { data: ministries, isLoading: mLoading } = useQuery({
    queryKey: ["ministries"],
    queryFn: getMinistries,
  });
  const { data: mealstubs, isLoading: msLoading } = useQuery({
    queryKey: [
      "meal-stubs",
      { dateFrom: dateRange.start, dateTo: dateRange.end },
    ],
  ...
  const filteredWorkers = useMemo(() => {
    return (
      workers
        ?.filter(
          (w) =>
            w.employmentType === "Full-Time" || w.employmentType === "On-Call",
        )
  ...
  const getMinistry = useCallback(
    (id: string) => ministries?.find((m) => m.id === id),
    [ministries],
  );

  const isAssignerFor = useCallback(
    (worker: Worker) => {
      ...
      return (
        primaryMinistry?.mealStubAssignerId === workerProfile.id ||
  ...
  const handleAssignSunday = async (worker: Worker) => {
    if (!isAssignerFor(worker)) {
      toast({
        variant: "destructive",
        title: "Access Denied",
        description: "You are not the meal stub assigner for this ministry.",
      });
      return;
    }
    ...
    if (!isSunday(today)) {
      toast({
        variant: "destructive",
        title: "Not Allowed",
        description: "Sunday meal stubs can only be assigned on Sundays.",
      });
      return;
    }
    ...
    try {
      ...
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to assign meal stub.",
      });
    }
  };
    ...
    <div className="space-y-4">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Tracking for Full-Time and On-Call workers. Weekdays: 5 limit,
            Sunday: 2 limit.
          </CardDescription>
        </CardHeader>
                        ...
                        <div className="flex flex-col items-center gap-1">
                          ...
                          <div className="w-20 h-1.5 bg-muted rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min((stats.weekdayCount / 5) * 100, 100)}%`,
                              }}
                            />
                          </div>
                        </div>
                        ...
                        <div className="flex flex-col items-center gap-1">
                          ...
                          <div className="w-20 h-1.5 bg-muted rounded-full overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min((stats.sundayCount / 2) * 100, 100)}%`,
                              }}
                            />
                          </div>
                        </div>
                      ...
                      <TableCell className="text-right">
                        {canAssign && (
                          <Button
                            size="sm"
                            variant="outline"
                            onClick={() => handleAssignSunday(worker)}
                          >
                            <PlusCircle className="mr-1 h-3 w-3" /> Assign
                            Sunday
                          </Button>
                        )}
                      </TableCell>
...
function ReservationsReport({ dateRange }: { dateRange: DateRange }) {
  ...
  const { data: reservations, isLoading } = useQuery({
    ...
    queryFn: () =>
      getBookings({ dateFrom: dateRange.start, dateTo: dateRange.end }),
  });
              ...
              {!isLoading &&
                reservations?.map((res) => (
                  <TableRow key={res.id}>
                    ...
                    <TableCell className="text-muted-foreground text-sm">
                      {res.workerProfileId
                        ? getWorkerName(res.workerProfileId)
                        : "N/A"}
                    </TableCell>
                    <TableCell className="text-muted-foreground text-sm">
                      {format(toJsDate(res.start), "PPp")}
                    </TableCell>
                    <TableCell>
                      <Badge
                        className={cn(
      ...
      <div className="max-w-7xl mx-auto w-full">
        <div className="flex flex-col gap-1 mb-6">
          ...
          <p className="text-sm text-muted-foreground">
            Monitor attendance, meal allocations, and room usage.
          </p>
        </div>
