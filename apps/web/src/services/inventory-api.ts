...
export async function getItems(ministryId: string | null, params: {
    ...
    if (ministryId) query = query.eq('group', ministryId);
...
export async function createItem(ministryId: string | null, payload: {
    ...
    const { data, error } = await supabase
        .from('InventoryItem')
        .insert({ ...payload, group: ministryId })
        .select()
        .single();
    if (error) throw error;
    return data;
...
export async function lookupItemByQR(payload: string) {
    const { data, error } = await supabase
        .from('InventoryItem')
        .select('*, category:InventoryCategory(name)')
        .or(`inventoryCode.eq.${payload},id.eq.${payload}`)
        .maybeSingle();
    if (error) throw error;
    if (!data) throw new Error('Item not found');
    return { item: { ...data, stock: data.quantity } };
}
